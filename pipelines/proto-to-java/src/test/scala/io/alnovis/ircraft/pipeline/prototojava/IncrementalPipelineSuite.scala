...
class IncrementalPipelineSuite extends munit.FunSuite:
  ...
  val pipeline: GenericProtoToCodePipeline = GenericProtoToCodePipeline(config, DirectJavaEmitter())
  ...
  tmpDir.test("first run generates all files and saves cache") { dir =>
    val module = IrModule("test", Vector(twoMessageSchema()))
    ...
    // Generic pipeline has no global files (ProtoWrapper etc. are in proto-wrapper-plugin)
  }
  ...
  tmpDir.test("changed message returns its files and global files") { dir =>
    val module1 = IrModule("test", Vector(twoMessageSchema()))
    ...
    val result = pipeline.executeIncremental(IrModule("test", Vector(schema2)), dir)
    ...
    val orderOnlyFiles = files.keySet.filter(_.contains("Order"))
  ...
  tmpDir.test("new message returns new files plus global files") { dir =>
    ...
    pipeline.executeIncremental(IrModule("test", Vector(schema1)), dir)
  ...
  tmpDir.test("pipeline errors propagate in incremental mode") { dir =>
    ...
    val result = pipeline.executeIncremental(IrModule("test", Vector(schema)), dir)
  ...
  tmpDir.test("cache is updated with new hashes after incremental run") { dir =>
    ...
    pipeline.executeIncremental(IrModule("test", Vector(schema2)), dir)
