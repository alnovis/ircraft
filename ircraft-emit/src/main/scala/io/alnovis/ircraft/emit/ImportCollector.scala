...
/** Walks a Decl tree and collects all imports via TypeMapping. */
object ImportCollector:

  def collect(decl: Decl, tm: TypeMapping): Set[String] =
    decl match
  ...
  private def collectStmt(s: Stmt, tm: TypeMapping): Set[String] = s match
    ...
    case Stmt.Let(_, t, init, _)    => tm.imports(t) ++ init.toSet.flatMap(e => collectExpr(e, tm))
    ...
    case Stmt.Match(e, cases) =>
      collectExpr(e, tm) ++
        cases.flatMap(mc =>
          collectPattern(mc.pattern, tm) ++ mc.guard.toSet.flatMap(g => collectExpr(g, tm)) ++ collectBody(mc.body, tm)
        )
  ...
  private def collectExpr(e: Expr, tm: TypeMapping): Set[String] = e match
    ...
    case Expr.Call(r, _, args, ta) =>
      r.toSet.flatMap(e => collectExpr(e, tm)) ++ args.flatMap(a => collectExpr(a, tm)) ++ ta.flatMap(tm.imports)
