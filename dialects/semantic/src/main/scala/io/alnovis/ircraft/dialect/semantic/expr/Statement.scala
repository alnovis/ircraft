...
object Statement:
  ...
  case class VarDecl(name: String, varType: TypeRef, initializer: Option[Expression] = None, isFinal: Boolean = false) extends Statement
  ...
  case class TryCatchStmt(tryBlock: Block, catches: List[CatchClause], finallyBlock: Option[Block] = None) extends Statement
