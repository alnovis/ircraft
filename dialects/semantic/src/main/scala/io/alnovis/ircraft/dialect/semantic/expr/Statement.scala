    ...
    def contentHash(a: Statement): Int =
      ...
      a match
        ...
        case VarDecl(n, t, init, fin)   =>
          ContentHash.combine(3, ContentHash.ofString(n), typeRefHash.contentHash(t),
            init.map(exprHash.contentHash).getOrElse(0), ContentHash.ofBoolean(fin))
        case Assignment(target, value)  => ContentHash.combine(4, exprHash.contentHash(target), exprHash.contentHash(value))
        case IfStmt(c, t, e)            =>
          ContentHash.combine(5, exprHash.contentHash(c), blockHash.contentHash(t),
            e.map(blockHash.contentHash).getOrElse(0))
        case ForEachStmt(v, t, it, b)   =>
          ContentHash.combine(6, ContentHash.ofString(v), typeRefHash.contentHash(t),
            exprHash.contentHash(it), blockHash.contentHash(b))
        case ThrowStmt(expr)            => ContentHash.combine(7, exprHash.contentHash(expr))
        case TryCatchStmt(t, cs, f)     =>
          ContentHash.combine(8, blockHash.contentHash(t),
