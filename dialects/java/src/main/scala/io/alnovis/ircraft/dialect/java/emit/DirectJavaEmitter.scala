...
class DirectJavaEmitter extends Emitter with EmitterUtils:
  ...
  private def emitInterface(i: InterfaceOp, level: Int): String =
    ...
    val ext =
      if i.extendsTypes.nonEmpty then
        s" extends ${i.extendsTypes.map(t => JavaTypeMapping.toJavaType(t)).mkString(", ")}"
      else ""
  ...
  private def emitClass(c: ClassOp, level: Int): String =
    ...
    val impl =
      if c.implementsTypes.nonEmpty then
        s" implements ${c.implementsTypes.map(t => JavaTypeMapping.toJavaType(t)).mkString(", ")}"
      else ""
  ...
  private def emitEnum(e: EnumClassOp, level: Int): String =
    ...
    val impl =
      if e.implementsTypes.nonEmpty then
        s" implements ${e.implementsTypes.map(t => JavaTypeMapping.toJavaType(t)).mkString(", ")}"
      else ""
  ...
  private def emitStmt(s: Statement, level: Int): String = s match
    ...
    case Statement.ForEachStmt(variable, varType, iterable, body) =>
      block(s"for (${JavaTypeMapping.toJavaType(varType)} $variable : ${emitExpr(iterable)})", level)(
    ...
    case Statement.TryCatchStmt(tryBlock, catches, finallyBlock) =>
      ...
      val catchStr = catches.map: c =>
        block(s"catch (${JavaTypeMapping.toJavaType(c.exceptionType)} ${c.variableName})", level)(
  ...
  private def emitExpr(e: Expression): String = e match
    ...
    case Expression.NewInstance(classType, args) =>
      s"new ${JavaTypeMapping.toJavaType(classType)}(${args.map(emitExpr).mkString(", ")})"
    ...
    if tps.isEmpty then ""
      ...
      val params = tps.map: tp =>
        if tp.upperBounds.nonEmpty then
          s"${tp.name} extends ${tp.upperBounds.map(JavaTypeMapping.toJavaType).mkString(" & ")}"
        else tp.name
      s"<${params.mkString(", ")}>"
