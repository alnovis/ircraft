...
class DirectScalaEmitter extends Emitter with EmitterUtils:
  ...
  def emit(module: Module): Map[String, String] =
    module.topLevel.collect { case f: FileOp => f }.flatMap: f =>
      f.types.map: op =>
  ...
  private def emitTrait(i: InterfaceOp, level: Int): String =
    ...
    val extendsClause = if i.extendsTypes.isEmpty then ""
  ...
  private def emitClass(c: ClassOp, level: Int): String =
    ...
    val withClause = if c.implementsTypes.isEmpty then ""
      else if c.superClass.isDefined then s" with ${c.implementsTypes.map(t => tm.toLanguageType(t)).mkString(" with ")}"
  ...
  private def emitEnum(e: EnumClassOp, level: Int): String =
    ...
    val extendsClause = if e.implementsTypes.isEmpty then ""
    ...
    val ctorParams = if hasConstructorParams then
    ...
    for c <- e.constants do
      val args = if c.arguments.isEmpty then ""
  ...
  private def emitMethod(m: MethodOp, level: Int): String =
    ...
    val params = if m.parameters.isEmpty then ""
  ...
  private def emitStmt(s: Statement, level: Int): String = s match
    ...
    case Statement.TryCatchStmt(tryBlock, catches, fin) =>
      ...
      if catches.nonEmpty then
        sb.append("\n" + indent(level, "catch"))
        for c <- catches do
          sb.append("\n" + indent(level + 1, s"case ${c.variableName}: ${tm.toLanguageType(c.exceptionType)} =>\n${emitBlock(c.body, level + 2)}"))
