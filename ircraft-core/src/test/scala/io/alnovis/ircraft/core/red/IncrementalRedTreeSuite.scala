...
class IncrementalRedTreeSuite extends munit.FunSuite:
  ...
  test("unchanged module produces empty diff"):
    val module = IrModule("test", Vector(leaf("a"), leaf("b")))
    val diff   = IncrementalRedTree.diff(module, module)
    assertEquals(diff, Vector.empty)

  test("single leaf change produces Modified"):
    val old  = IrModule("test", Vector(leafWithAttr("a", "v", "1")))
    val now  = IrModule("test", Vector(leafWithAttr("a", "v", "2")))
  ...
  test("added operation produces Added"):
    val old  = IrModule("test", Vector(leaf("a")))
    val now  = IrModule("test", Vector(leaf("a"), leaf("b")))
    val diff = IncrementalRedTree.diff(old, now)
    assertEquals(diff.size, 1)
  ...
  test("removed operation produces Removed"):
    val old  = IrModule("test", Vector(leaf("a"), leaf("b")))
    val now  = IrModule("test", Vector(leaf("a")))
    val diff = IncrementalRedTree.diff(old, now)
    assertEquals(diff.size, 1)
    assertEquals(diff.head.path, Vector(1))
  ...
  test("nested change reports correct path"):
    val old  = IrModule("test", Vector(container("root", leafWithAttr("x", "v", "1"))))
    val now  = IrModule("test", Vector(container("root", leafWithAttr("x", "v", "2"))))
    val diff = IncrementalRedTree.diff(old, now)
  ...
  test("unchanged sibling not in diff"):
    val old  = IrModule("test", Vector(leaf("a"), leafWithAttr("b", "v", "1")))
    val now  = IrModule("test", Vector(leaf("a"), leafWithAttr("b", "v", "2")))
    val diff = IncrementalRedTree.diff(old, now)
