...
class RedNodeSuite extends munit.FunSuite:
  ...
  test("root offset is 0"):
    val root = RedTree.from(Module("test", Vector(leaf("a"), leaf("b"))))
    assertEquals(root.offset, 0)
  ...
  test("nested offsets are cumulative"):
    ...
    val root     = RedTree.from(Module("test", Vector(inner)))
  ...
  test("nextSibling navigates forward"):
    val root = RedTree.from(Module("test", Vector(leaf("a"), leaf("b"), leaf("c"))))
  ...
  test("find locates matching descendant"):
    ...
    val root  = RedTree.from(Module("test", Vector(leaf("other"), inner)))
  ...
  test("collectWithContext finds all matching nodes"):
    val root  = RedTree.from(Module("test", Vector(leaf("a"), container("mid", leaf("a")), leaf("b"))))
  ...
  test("contentHash delegates to green"):
    val op   = leaf("x")
    val root = RedTree.from(Module("test", Vector(op)))
